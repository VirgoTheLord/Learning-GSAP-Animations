...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function togglePreview(index) {
    ...
    transformState.forEach((state) => {
      ...
      state.currentX = state.currentY = state.targetX = state.targetY = 0;
    });
  ...
  function handleResize() {
    ...
    if (!isPreviewActive) {
      parallexState.targetX = 0;
      parallexState.targetY = 0;
      parallexState.targetZ = 0;
      parallexState.currentX = 0;
      parallexState.currentY = 0;
      parallexState.currentZ = 0;
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    cards.forEach((card, index) => {
      ...
      if (distance < config.sensitivity && !config.isMobile) {
        ...
        transformState[index].targetRotation = 100 * flipFactor;
  ...
  function animate() {
    if (!isPreviewActive && !isTransitioning) {
      parallexState.currentX +=
        (parallexState.targetX - parallexState.currentX) * config.lerpFactor;
      parallexState.currentY +=
        (parallexState.targetY - parallexState.currentY) * config.lerpFactor;
      parallexState.currentZ +=
        (parallexState.targetZ - parallexState.currentZ) * config.lerpFactor;
